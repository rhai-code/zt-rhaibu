...
**Reference-reading budget:** read at most ONE reference catalog, and only its
structure-relevant sections — the include block, the mandatory `config:`/
`cloud_provider:` vars, and one representative `ocp4_workload_*` variable block
(≈50 lines). Never read a full 300+ line example when only confirming current
patterns. Prefer the real catalog over the bundled examples below. In headless
/ orchestrator mode (or whenever the main conversation is already busy), run the
entire Step 1-2 reference read inside the config-writer subagent and return only
the patterns it relied on — never the raw YAML (WORKSHOP-COMMON-RULES §7a).

**Step 2 — Fall back to bundled examples if no real catalog found (read ONLY the
one matching the infra type — never all of them):**
...
**`#include` lines (boilerplate, event/AWS/LiteMaaS includes, duplicate-include loop
guard) and workload-specific secrets:** see
`${CLAUDE_SKILL_DIR}/references/common-yaml-rules.md`.
...
**Full `__meta__` output template:** see
`${CLAUDE_SKILL_DIR}/references/common-yaml-rules.md` — the complete `__meta__` block,
and the note that no-event catalogs omit the `Brand_Event` label and event keywords.
